...
import com.example.climacool.databinding.FragmentLayoutRegisterBinding
import com.example.climacool.viewModel.SignUpViewModel
...
class Layout_register : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (activity is FragmentComunicator) {
        ...
        viewModel.registrationSuccess.observe(viewLifecycleOwner) { success ->
            ...
            if (success) {
                val userId = viewModel.isUserCreted.value ?: run {
                    Toast.makeText(
                        activity,
                        "Error: No se obtuvo el ID de usuario",
                        Toast.LENGTH_SHORT
                    ).show()
                    return@observe
                }
